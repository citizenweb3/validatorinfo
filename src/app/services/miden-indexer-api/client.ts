...
import { MidenIndexerRequestOptions } from './types';


const { logDebug, logError } = logger('miden-indexer-client');
...
export const buildUrl = (path: string, params?: QueryParams): string => {
  const base = process.env.MIDEN_INDEXER_BASE_URL || DEFAULT_BASE_URL;
...
const buildHeaders = (customHeaders?: HeadersInit): HeadersInit => {
  ...
  const apiToken = process.env.MIDEN_INDEXER_API_TOKEN;
  if (apiToken) {
...
const buildNextOptions = (options?: MidenIndexerRequestOptions): RequestInit => {
...
const request = async <T>(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new HttpError(
        response.status,
        `HTTP ${response.status} ${response.statusText}: ${errorBody} (URL: ${url})`,
      );
    }
...
export const getBaseUrl = (): string => {
  return process.env.MIDEN_INDEXER_BASE_URL || DEFAULT_BASE_URL;
};
