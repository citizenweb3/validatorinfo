...
import { CosmosIndexerRequestOptions } from './types';


const { logDebug, logError } = logger('cosmos-indexer-client');
...
export const buildUrl = (path: string, params?: QueryParams): string => {
  const baseUrl = `${process.env.COSMOS_INDEXER_BASE_URL}${path}`;
...
const buildNextOptions = (options?: CosmosIndexerRequestOptions): RequestInit => {
...
export const getBaseUrl = (): string => {
  return process.env.COSMOS_INDEXER_BASE_URL || '';
};
