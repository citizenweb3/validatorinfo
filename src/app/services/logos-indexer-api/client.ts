...
import { LogosIndexerRequestOptions } from './types';


const { logDebug, logError } = logger('logos-indexer-client');
...
export const buildUrl = (path: string, params?: QueryParams): string => {
  const baseUrl = `${process.env.LOGOS_INDEXER_BASE_URL}${path}`;
...
const buildHeaders = (customHeaders?: HeadersInit): HeadersInit => {
  ...
  const apiToken = process.env.LOGOS_INDEXER_API_TOKEN;
  if (apiToken) {
...
const buildNextOptions = (options?: LogosIndexerRequestOptions): RequestInit => {
...
export const getBaseUrl = (): string => {
  return process.env.LOGOS_INDEXER_BASE_URL || '';
};
