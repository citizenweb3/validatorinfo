...
import { AtomoneIndexerRequestOptions } from './types';


const { logDebug, logError } = logger('atomone-indexer-client');
...
export const buildUrl = (path: string, params?: QueryParams): string => {
  const baseUrl = `${process.env.ATOMONE_INDEXER_BASE_URL}${path}`;
...
const buildHeaders = (customHeaders?: HeadersInit): HeadersInit => {
  ...
  const apiKey = process.env.ATOMONE_INDEXER_API_KEY;
  if (apiKey) {
...
const buildNextOptions = (options?: AtomoneIndexerRequestOptions): RequestInit => {
...
export const healthCheck = async (): Promise<boolean> => {
  try {
    await get('/api/v1/health', null, { timeout: 5000, cache: 'no-store' });
    return true;
  } catch {
    return false;
  }
...
export const getBaseUrl = (): string => {
  return process.env.ATOMONE_INDEXER_BASE_URL || '';
};
