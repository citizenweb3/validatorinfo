...
const StakingResults: FC<OwnProps> = ({ values, chain }) => {
  ...
  if (!values) {
    return <div className="mt-2.5 text-base">Please select validator and chain!</div>;
  }
  ...
  return (
    <div className="border-b border-l border-bgSt p-4 text-base">
      <div className="ml-24 flex h-3 flex-row items-end text-xs uppercase">
        ...
        <div className="h-3">Token</div>
      </div>
      <div className="mt-3">Enter Staking Amount</div>
      <div className="relative">
        <div className="absolute left-1 top-0 font-retro text-highlight">$</div>
        <input
      ...
      {isNaN(+amount) ? (
        <div className="mt-2.5 text-primary">Wrong amount</div>
      ) : (
