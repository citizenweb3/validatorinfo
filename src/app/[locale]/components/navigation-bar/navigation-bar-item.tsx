...
const NavigationBarItem: FC<OwnProps> = ({ item: { name, href, icon, iconHovered }, isOpened }) => {
  ...
  return (
    <Link
      href={href}
      className={`${isOpened ? 'min-h-10 w-[15.5rem]' : 'h-10 w-10 hover:border-highlight'} !active:max-h-7 hover:bg-bgHover group relative flex cursor-pointer flex-row items-center overflow-hidden border-r border-t border-bgSt text-sm font-semibold shadow-button transition-width duration-300 active:top-1 active:border-transparent active:bg-background active:shadow-none`}
    >
