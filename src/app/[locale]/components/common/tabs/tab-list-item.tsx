...
const TabListItem: FC<OwnProps> = ({
  ...
  // Disabled tab: no data for this entity (e.g. governance/revenue for a pool). Kept visible for
  // layout parity but rendered blurred and non-interactive (no navigation).
  if (disabled) {
    return (
      <div
        aria-disabled="true"
        className={`pointer-events-none border-r border-t border-bgSt bg-table_row ${menuButtonRaisedShadow} group relative mt-12 flex min-h-36 min-w-0 w-full flex-grow cursor-default flex-row items-center justify-center overflow-hidden p-0.5 text-sm opacity-60 blur-[1.5px] sm:mt-12 sm:min-h-20 md:mt-0 md:min-h-10`}
      >
        {content}
      </div>
    );
