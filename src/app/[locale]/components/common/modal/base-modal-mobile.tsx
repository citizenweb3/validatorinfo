...
const BaseModalMobile: FC<PropsWithChildren<OwnProps>> = ({
    ref,
    opened,
    children,
    onClose,
    ...
    <div ref={modalRef} className={`${opened ? 'block' : 'hidden'} ${isRelative ? 'relative' : ''}`}>
      <div className={`${className} ${isRelative ? 'absolute' : 'fixed'} z-40 bg-background shadow-3xl`} style={style}>
        <div className={`${!hideClose && 'pt-6'} relative p-3`}>
          {!hideClose && (
            <div
              className={`absolute right-0 top-0 z-50 w-24 h-24 bg-close bg-contain hover:bg-close_h active:bg-close_a`}
              onClick={(e) => {
                e.stopPropagation();
                e.preventDefault();
                onClose();
          ...
          <div className={`${maxHeight}`}>{children}</div>
        </div>
      </div>
    </div>
  );
