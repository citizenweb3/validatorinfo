...
const CosmosBlockInformation: FC<OwnProps> = async ({ chain, hash }) => {
  const t = await getTranslations('BlockInformationPage');

  if (!/^\d+$/.test(hash)) {
    // Cosmos indexer doesn't expose lookup-by-hash; the table links by height.
    notFound();
  }
...
export default CosmosBlockInformation;
