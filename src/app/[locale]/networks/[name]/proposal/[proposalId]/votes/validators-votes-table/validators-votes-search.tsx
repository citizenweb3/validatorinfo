...
const ValidatorsVotesSearch: FC<OwnProps> = ({ chainName, proposalId }) => {
  ...
  const fetchResults = async (q: string) => {
    ...
    try {
      const resp = await fetch(
        `/api/proposal_validators_votes?chainName=${chainName}&proposalId=${proposalId}&q=${encodeURIComponent(q)}`,
      );
