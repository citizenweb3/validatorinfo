...
const CommitteeSizeDisplay: FC<OwnProps> = async ({ chainName }) => {
  const t = await getTranslations('NetworkPassport');
  const committeeSize = await aztecContractService.getCommitteeSize(chainName);
  ...
  return (
    <div className="mt-2 flex w-full bg-table_row hover:bg-bgHover">
      ...
      <div
        ...
        <Link href={`/networks/${chainName}/governance`}>
          {committeeSize}
        </Link>
      </div>
    </div>
...
export default CommitteeSizeDisplay;
