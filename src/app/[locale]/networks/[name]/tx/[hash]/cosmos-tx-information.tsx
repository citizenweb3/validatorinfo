...
const formatCosmosTimestamp = (iso: string): string =>
  new Date(iso).toISOString().replace('T', ' ').replace(/\.\d{3}Z$/, ' UTC');

const CosmosTxInformation: FC<OwnProps> = async ({ chain, hash }) => {
    ...
    },
    {
      ...
      value: (
        <div className="flex items-center gap-2">
          <span className="font-handjet text-lg">{formatCosmosTimestamp(tx.time)}</span>
          <CopyButton value={formatCosmosTimestamp(tx.time)} />
        </div>
      ),
    },
    {
...
export default CosmosTxInformation;
