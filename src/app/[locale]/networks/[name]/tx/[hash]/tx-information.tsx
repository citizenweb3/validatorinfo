...
const TxInformation: FC<OwnProps> = async ({ chain, hash }) => {
  ...
  const formatData = (title: string, data: number | string) => {
    switch (title) {
      ...
      case 'block height':
        return (
          <Link href={`/networks/${chain?.name}/block/${data}`} className="font-handjet text-lg text-highlight hover:underline">
    ...
    <div className="mt-2">
      <div className="flex justify-between mb-8 ml-5">
        ...
        <div className="flex items-center">
          <RoundedButton href={`/networks/${chain?.name}/tx`} className="font-handjet text-lg mb-4 active:mb-3">
