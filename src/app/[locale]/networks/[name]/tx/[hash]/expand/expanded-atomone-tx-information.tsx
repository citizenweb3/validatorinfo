...
import { AtomoneTxEvent } from '@/services/atomone-indexer-api';
...
const ADDRESS_PREFIXES = ['atone1', 'atonevaloper1', 'atonepub1', 'atonevalconspub1'];
...
const groupEventsByMsgIndex = (events: AtomoneTxEvent[]): Map<number, AtomoneTxEvent[]> => {
  const grouped = new Map<number, AtomoneTxEvent[]>();
...
const EventCard: FC<{ event: AtomoneTxEvent; chainName: string }> = ({ event, chainName }) => {
...
const ExpandedAtomoneTxInformation: FC<OwnProps> = async ({ chain, hash }) => {
...
export default ExpandedAtomoneTxInformation;
