type DataPoint = {
  ...
  export const generateDataForTotalBar = (startDate: Date, endDate: Date, chartType: string, numBars: number): { tvs: DataPoint[]; rewards: DataPoint[] } => {
    // Generate random value between 0 and 5 trillion
    const getRandomValue = () => Math.floor(Math.random() * 5 * 10**12); // Values between 0 and 5 trillion
  
    const generateDataset = (startDate: Date, numBars: number, interval: number) => {
      const dataset = [];
    ...
    const generatedData = generateDataset(startDate, numBars, interval);
