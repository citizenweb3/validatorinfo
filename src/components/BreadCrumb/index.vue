...
const fullRouteResolved = computed(() => props.fullRoute?.flatMap((r: any) => {
  return [
    ...(r.meta.parentPath ? [r.meta.parentPath] : []),
    {
    title: r.meta.title?.includes(':') ? route.currentRoute.value.params[r.meta.title.split(':')[1]] : r.meta.title,
    href: r.path.split('/').map((d: string) => {
