...
const { logError, logInfo } = logger('get-slashing-params');

const updateChainSlashingParams = async (chainNames: string[]) => {
  for (const chainName of chainNames) {
    ...
    try {
      ...
      if (dbChain.hasValidators) {
        ...
        const params = await chainMethods.getSlashingParams(chainParams);
        logInfo(`${chainName} Slashing params: ${JSON.stringify(params)}`);
      ...
      logError("Can't fetch slashing params: ", e);
    }
  }
};

export default updateChainSlashingParams;
