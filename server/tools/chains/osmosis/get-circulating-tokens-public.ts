...
const { logError } = logger('osmosis-circulating-tokens-public');

const getCirculatingTokensPublic: GetCirculatingTokensPublic = async (chain: AddChainProps) => {
  const response = await fetch('https://public-osmosis-api.numia.xyz/supply/v1/metrics?metric=circulatingSupply');
