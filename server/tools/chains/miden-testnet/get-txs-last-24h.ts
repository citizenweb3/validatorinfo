...
/**
 * Miden indexer has no `/tx-effects-last-24h`-style endpoint, and paginating
 * the full tx list per cron tick is far too expensive. Use the daily snapshot
 * table that powers txs30d: read yesterday's totalTxs snapshot (UTC), subtract
 * from current total. Returns null on the first run (no snapshot yet).
 */
const getTxsLast24h: GetTxsLast24h = async (dbChain) => {
  ...
  if (!Number.isInteger(currentTotal) || currentTotal < 0) return null;
