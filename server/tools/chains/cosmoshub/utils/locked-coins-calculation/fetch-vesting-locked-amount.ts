...
const { logWarn } = logger('cosmos-fetch-vesting-tokens');
  ...
  while (true) {
    ...
    if (response.accounts && response.accounts.length > 0) {
      for (const account of response.accounts) {
        if (account.type?.includes('VestingAccount') || account.type?.includes('PermanentLockedAccount')) {
          if (account.address) vesting.add(account.address);
        }
      }
    }
