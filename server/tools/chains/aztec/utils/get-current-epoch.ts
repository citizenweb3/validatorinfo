...
const { logError } = logger('get-current-epoch');

const getCurrentEpoch = async (chainName: string) => {
  ...
  try {
    ...
    const currentEpoch = await readContractWithFailover<bigint>(
      l1RpcUrls,
      {
      ...
      `${chainName}-get-current-epoch`,
    );

    return currentEpoch;
  } catch (e) {
    logError(`${chainName} - can't fetch current epoch`);
    return null;
  }
};
