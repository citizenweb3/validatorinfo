...
const getProposals: GetProposalsFunction = async (chain) => {
  ...
  try {
    ...
    const url = `/atomone/gov/v1/proposals?pagination.limit=100`;
    ...
    while (true) {
      ...
      try {
          ...
          if (proposal.status === ProposalStatus.PROPOSAL_STATUS_VOTING_PERIOD) {
            try {
              const tallyResponse = await fetchChainData<{
                tally: TallyResult;
              }>(chain.name, 'rest', `/cosmos/gov/v1/proposals/${proposal.id}/tally`);
        ...
        logInfo(`Fetched ${proposals.length} Proposals for ${chain.name}, next_key: ${response.pagination?.next_key}`);
